...
NS_INLINE UIImage *SDScaledImageForPath(NSString *path, NSData *imageData)
{
    ...
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)])
    {
        ...
        if (path.length >= 8)
        {
            // Search  at the end of the string, before a 3 to 4 extension length (only if key len is 8 or more  + 4 len ext)
            NSRange range = [path rangeOfString:@"" options:0 range:NSMakeRange(path.length - 8, 5)];
