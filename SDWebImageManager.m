...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate imgSizeWide:(float)sizeWide

{
    ...
    if (!url || !delegate || (!(0 & SDWebImageRetryFailed) && [failedURLs containsObject:url]))
    {
        return;
    }
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:delegate, @"delegate", url, @"url", [NSNumber numberWithInt:0], @"options",[NSNumber numberWithFloat:sizeWide], @"wide", nil];
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    NSNumber *tempNum = [downloader.userInfo objectForKey:@"wide"];
    UIImage  *newImage = image;
    NSString *memKey = [downloader.url absoluteString];
    
    if ((image)&&(tempNum!=nil)) {
        if ([memKey rangeOfString:@"?"].length > 0) {
            if ([memKey rangeOfString:@"width"].length == 0) {
                memKey = [memKey stringByAppendingFormat:@"&width=%@",tempNum];
            }
        }else{
            memKey = [memKey stringByAppendingFormat:@"?width=%@",tempNum];
        }
        ...
        if (image.size.width!=0) {
            CGSize newSize = CGSizeMake([tempNum doubleValue], [tempNum doubleValue]*image.size.height/image.size.width);
