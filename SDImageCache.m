...
{
    NSNumber *tempNum = [info objectForKey:@"wide"];
    if (!delegate)
    {
        return;
    }
    ...
    if (tempNum) {
        if ([key rangeOfString:@"?"].length > 0) {
            if ([key rangeOfString:@"width"].length == 0) {
                memKey = [key stringByAppendingFormat:@"&width=%@",tempNum];
            }
        }else{
            memKey = [key stringByAppendingFormat:@"?width=%@",tempNum];
        }
    }
    UIImage *image = [memCache objectForKey:memKey];
    ...
    if (image)
    {
        ...
        if ([delegate respondsToSelector:@selector(imageCache:didFindImage:forKey:userInfo:)])
        {
            ...
            [delegate imageCache:self didFindImage:image forKey:memKey userInfo:info];
        }
        return;
    }

    NSMutableDictionary *arguments = [NSMutableDictionary dictionaryWithCapacity:3];
    [arguments setObject:memKey forKey:@"key"];
